...
class GaussianProcessBase(object):
    ...
    def log_marginal_lengthscale(self,X,Y,lengthscale,noise_delta):
        """
        Compute Log Marginal likelihood of the GP model w.r.t. the provided lengthscale
        """

        def compute_log_marginal(X,Y,lengthscale,noise_delta):
            # compute K
            ur = unique_rows(X)
            myX=X[ur]
            myY=Y[ur]
        ...
        if np.isscalar(lengthscale):
            logmarginal=compute_log_marginal(X,Y,lengthscale,noise_delta)
            return logmarginal
        ...
        if not isinstance(lengthscale,list) and len(lengthscale.shape)==2:
            ...
            for idx in range(lengthscale.shape[0]):
                logmarginal[idx]=compute_log_marginal(X,Y,lengthscale[idx],noise_delta)
        else:
            logmarginal=compute_log_marginal(X,Y,lengthscale,noise_delta)
    ...
    def optimize_lengthscale_SE_maximizing(self,previous_theta,noise_delta):
        ...
        bounds_lengthscale_max=1
        #mySearchSpace=np.asarray([bounds_lengthscale_min,bounds_lengthscale_max]).T
       
        mybounds=np.asarray([[bounds_lengthscale_min,bounds_lengthscale_max]])
        
        lengthscale_tries = np.random.uniform(mybounds[:, 0], mybounds[:, 1],size=(20, mybounds.shape[0]))


        # evaluate
        self.flagOptimizeHyperFirst=0 # for efficiency

        logmarginal_tries=self.log_marginal_lengthscale(self.X,self.Y,lengthscale_tries,noise_delta)
        #print logmarginal_tries
        ...
        for i in range(1):
            res = minimize(lambda x: -self.log_marginal_lengthscale(self.X,self.Y,x,noise_delta),lengthscale_init_max,
                           bounds=mybounds,method="L-BFGS-B",options=myopts)#L-BFGS-B
            if 'x' not in res:
                val=self.log_marginal_lengthscale(self.X,self.Y,res,noise_delta)    
            else:
                val=self.log_marginal_lengthscale(self.X,self.Y,res.x,noise_delta)  
