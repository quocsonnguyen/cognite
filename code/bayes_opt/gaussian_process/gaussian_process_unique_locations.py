...
# from the paper https://arxiv.org/pdf/2201.12909.pdf
# Scaling Gaussian Process Optimization
# by Evaluating a Few Unique Candidates Multiple Times


class GP_UniqueLocations(GaussianProcessBase):
    ...
    def __init__ (self,param):
        ...
        # self.X_unique, self.Y_unique, \
        #     self.repeated_counts = self.find_unique_rows(self.X,self.Y)
    ...
    def find_unique_rows(self, X, y):
        ...
        idxRepeated = np.column_stack ( np.where(distance<0.0001*self.dim) )
        ...
        for iRow, xx in enumerate(X):
            ...
            idxCol = np.where(distance[iRow,:] < 0.0001*self.dim )
            
            new_X = np.mean( X[idxCol,:], axis=0)
            new_Y = np.mean(y[idxCol])
            
            X_unique.append(new_X)
            y_unique.append(new_Y)
            repeated_counts.append(len(idxCol))

            
        repeated_counts = np.asarray(repeated_counts).reshape((-1,1))
        y_unique = np.asarray(y_unique).reshape((-1,1))
        X_unique = np.asarray(X_unique).reshape((-1,self.dim))

        return X_unique, y_unique, repeated_counts
        

    def kernel_dist(self, a,b,lengthscale):
        super(GP_UniqueLocations,self).kernel_dist(a,b,lengthscale)
        
    def ARD_dist_func(self,A,B,length_scale):
        super(GP_UniqueLocations,self).ARD_dist_func(A,B,length_scale)
    ...
    def fit(self,X,Y):
        ...
        self.X, self.Y, \
            self.repeated_counts = self.find_unique_rows(self.X_full,self.Y_full)
        ...
        if self.kernel_name=='SE':
            ...
            self.KK_x_x=np.exp(-np.square(Euc_dist)/self.lengthscale)+np.eye(len(X))*self.noise_delta/self.repeated_counts
        else:
            KK=pdist(self.X,lambda a,b: self.kernel_dist(a,b,self.lengthscale)) 
            KK=squareform(KK)
            
            #self.KK_x_x=KK+np.eye(self.X.shape[0])*(1+self.noise_delta)
            self.KK_x_x = KK+np.eye(self.X.shape[0])*self.noise_delta*1.0/self.repeated_counts
    ...
    def leave_one_out_lengthscale(self,lengthscale,noise_delta):
        ...
        def compute_loo_predictive(X,lengthscale,noise_delta):
            ...
            for i in range(0,D.shape[0]):
                ...
                gp=GP_UniqueLocations(gp_params)
    ...
    def optimize_lengthscale_SE_loo(self,previous_theta,noise_delta):
        ...
        return super(GP_UniqueLocations,self).optimize_lengthscale_SE_loo(previous_theta,noise_delta)
    ...
    def optimize_lengthscale_SE_maximizing(self,previous_theta,noise_delta):
        ...
        return super(GP_UniqueLocations,self).optimize_lengthscale_SE_maximizing(previous_theta,noise_delta)
    ...
    def compute_incremental_cov_matrix(self,X,newX):
        ...
        super(GP_UniqueLocations,self).compute_incremental_cov_matrix(X,newX)
        """
    ...
    def compute_var(self,X,xTest):
        ...
        return super(GP_UniqueLocations,self).compute_var(X,xTest)
