    ...
    def predict(self,xTest,eval_MSE=True):
        ...
        if self.kernel_name=='SE':
            ...
            if xTest.shape[0]<=800:
                ...
                KK_xTest_xTrain=cdist(xTest,X,lambda a,b: self.kernel_dist(a,b,self.lengthscale))
                KK_xTest_xTest=cdist(xTest,xTest,lambda a,b: self.kernel_dist(a,b,self.lengthscale))+np.eye(xTest.shape[0])*self.noise_delta
            """
        ...
        var=np.ones(xTest.shape[0])-np.diag(np.dot(v.T,v))
